...
/**
 * G1. updateWellnessUI
 * Refreshes the three wellness widgets after every completed session.
 * All DOM ids are preserved from v7.2 to avoid breaking the HTML.
 *
 * @param {number} rawStabilityScore  – raw (pre-calibration) score
 *                                      from the session just saved
 */
function updateWellnessUI(rawStabilityScore) {
  ...
  // Derive display score (calibrated if baseline is ready)
  const allSessions = getSessions();
  const { baselineMean, calibrated } = getBaseline(allSessions);
  const displayScore = calibrated
    ? calibrateScore(rawStabilityScore, baselineMean)
    : safeClamp(rawStabilityScore);

  // ── WIDGET 1 · Stability Score ──────────────────────────────────
  if (scoreEl) {
    scoreEl.textContent = displayScore;
    scoreEl.classList.remove('score-high', 'score-mid', 'score-low');
    if      (displayScore >= 75) scoreEl.classList.add('score-high');
    else if (displayScore >= 50) scoreEl.classList.add('score-mid');
    else                         scoreEl.classList.add('score-low');

    // Restart CSS reveal animation
    scoreEl.classList.remove('score-revealed');
    void scoreEl.offsetWidth; // force reflow
    scoreEl.classList.add('score-revealed');
  }

  // SVG ring (circumference = 2π × 36 ≈ 226 px)
  if (ringFill) {
    const CIRC = 226;
    ringFill.style.strokeDashoffset = CIRC - (displayScore / 100) * CIRC;
  }
  if (ringLabel) ringLabel.textContent = displayScore;

  // Contextual message
  if (messageEl) {
    const inBaselinePhase = !calibrated;
    if (inBaselinePhase) {
      const rem = WELLNESS_CONFIG.BASELINE_N - allSessions.length;
      messageEl.textContent = rem > 0
        ? `Calibrating your baseline… ${rem} session${rem !== 1 ? 's' : ''} to go.`
        : 'Baseline established — personalised scoring active.';
    } else if (displayScore >= 80) {
      messageEl.textContent = 'Excellent emotional balance today.';
    } else if (displayScore >= 65) {
      messageEl.textContent = 'Good stability. Keep practicing.';
    } else if (displayScore >= 45) {
      messageEl.textContent = 'Building steadily — stay consistent.';
    } else {
      messageEl.textContent = 'Your system needs recovery. Stay consistent.';
    }
  }
  ...
  if (streakEl) {
    ...
    if (streak >= 2) {
      streakEl.textContent = `🔥 ${streak}-Day Stability Streak`;
      streakEl.style.display = 'inline-flex';
    } else if (streak === 1) {
      streakEl.textContent = '✨ First day — come back tomorrow!';
      streakEl.style.display = 'inline-flex';
    } else {
  ...
  // ── WIDGET 3 · Weekly Forecast ──────────────────────────────────
  const forecast = calculateWeeklyForecast();

  if (!forecast) {
    if (forecastEl) {
      const rem = WELLNESS_CONFIG.FORECAST_MIN - allSessions.length;
      forecastEl.textContent = rem > 0
        ? `Complete ${rem} more session${rem !== 1 ? 's' : ''} to unlock your weekly outlook.`
        : 'Complete more sessions to unlock your weekly outlook.';
  ...
  // Forecast copy
  if (forecastEl) {
    const confColor = forecast.confidence.label === 'High'   ? '#10b981' :
                      forecast.confidence.label === 'Medium' ? '#f59e0b' : '#ef4444';

    const baseNote = forecast.baselineCalibrated
      ? ` <span style="font-size:0.7rem;opacity:0.6;">(baseline: ${forecast.baselineMean})</span>`
      : '';

    const slopeStr = (forecast.slope >= 0 ? '+' : '') + forecast.slope;

    forecastEl.innerHTML =
      `Your trend is <strong>${forecast.direction}</strong>${baseNote}. ` +
      `5-session average: <strong>${forecast.ma5}</strong> · ` +
      `Trend: <strong>${slopeStr} pts/session</strong>. ` +
      `Score may reach ` +
      `<span class="forecast-projected-score">${forecast.projected}</span>` +
      ` this week. ` +
      `<span style="font-size:0.7rem;font-weight:600;color:${confColor};">` +
  ...
  if (barsEl && barsWrap) {
    ...
    // Render calibrated scores so bars match the displayed metric
    const chartScores = forecast.calibratedScores;
    const maxVal      = Math.max(...chartScores, forecast.projected, 1);

    chartScores.forEach((score, i) => {
      const bar       = document.createElement('div');
      bar.className   = 'forecast-mini-bar';
      bar.style.height = Math.round((score / maxVal) * 100) + '%';
      bar.title       = `Session ${i + 1}: ${Math.round(score)}`;
    ...
    projBar.title      = `Projected: ${forecast.projected}`;
    barsEl.appendChild(projBar);
  }
}
...
(function () {
  ...
  function updateSessionTimer() {
    ...
    if (el) {
      el.innerHTML = `running <span style="opacity:0.7;">•</span> ${formatElapsedTime(Date.now() - sessionStartTime)}`;
    }
  }
  ...
  allowMicBtn.addEventListener('click', async () => {
    ...
    if (ok) { await loadDevices(); alert('Microphone permission granted.'); }
  ...
  allowCamBtn.addEventListener('click', async () => {
    ...
    catch (e) { console.error(e); alert('Camera permission is required.'); }
  });
    ...
    try {
      ...
      if (cielabEnabled) {
        ...
        if (window.__cielabLogCounter++ % 30 === 0) {
          console.log(`[CIELAB ACTIVE] L:${meanL.toFixed(1)} a:${meanA.toFixed(1)} b:${meanB.toFixed(1)}`);
        }
  ...
  function onFaceResults(results) {
    ...
    if (window.__sessionActive && rppgSignal.green.length >= minBuf && bpmUpdateCounter % RPPG_CONFIG.UPDATE_INTERVAL === 0) {
      const posSignal  = applyPosAlgorithm(
        rppgSignal.red.slice(-minBuf),
        rppgSignal.green.slice(-minBuf),
        rppgSignal.blue.slice(-minBuf)
      );
  ...
  function startVisualizer() {
    ...
    function resize() {
      ...
      waveCanvas.height = Math.floor(260 * dpr);
      waveCanvas.style.width  = w + 'px';
      waveCanvas.style.height = '260px';
    ...
    function draw() {
      ...
      if (!scriptedGrow) {
        ...
        breathingCircle.style.transform = `scale(${(breathingBaseScale || 0.8) * (1 + amplitude * 0.12)})`;
      }
      ...
      if (micLvl)    micLvl.textContent     = micPct + '%';
  ...
  async function startSession() {
    ...
    sessionBreathScores   = []; sessionEyeScores   = [];
    sessionHeadScores     = []; sessionGazeScores  = [];
    ...
    startAmbience();
    runTimedBreathing();
    try { speak('', 'Welcome. Follow the voice.'); } catch (_) {}
  }
  ...
  function computeFinalResultAndShow() {
    ...
    const avgBPM  = sessionBPMValues.length > 0
      ? Math.round(avgArr(sessionBPMValues))
      : (window.__lastBPM || 0);

    const breathPct = Math.round(result.b * 100);
    const meditPct  = Math.round(result.meditationScore);

    // ── Compute raw stability score (pre-baseline) ────────────────────
    const rawStability = calculateStability(avgBPM, breathPct, meditPct);

    // ── Persist raw score (baseline calibration applied at read-time) ─
    saveSession(rawStability);

    // ── Derive calibrated display score for history table ─────────────
    const allSess = getSessions();
    const { baselineMean, calibrated } = getBaseline(allSess);
    const displayStability = calibrated
      ? calibrateScore(rawStability, baselineMean)
      : rawStability;

    // ── Append to session display history ─────────────────────────────
    sessionHistory.push({
  ...
  function renderHistory() {
    ...
    sessionHistory.slice().reverse().forEach(s => {
      ...
      tr.innerHTML = `
        ...
        <td>${s.meditation}%</td>
        <td>${s.overall}%</td>
        <td><strong>${s.stability !== undefined ? s.stability + '%' : '—'}</strong>
            ${s.cielabUsed ? '<span style="font-size:0.65rem;color:#f59e0b;">🔬</span>' : ''}</td>
      `;
  ...
  bgAudio.addEventListener('error', e  => {
    ...
    if (window.__sessionActive) alert('Selected audio track failed to load. Please try another track.');
  });
  ...
  function startAmbience() {
    ...
    bgAudio.play().catch(err => console.warn('Audio play failed/blocked:', err));
  ...
  (function initWellnessDisplay() {
    ...
    if (last.date.slice(0, 10) === todayKey) {
      // Replay today's final score through the full renderer
      updateWellnessUI(last.score);
      return;
    }

    // Different day — restore only streak + forecast
    const streakEl   = document.getElementById('streakDisplay');
    const forecastEl = document.getElementById('weeklyForecast');
    const barsWrap   = document.getElementById('forecastBarsWrap');
    const barsEl     = document.getElementById('forecastBars');
    ...
    if (forecast && forecastEl) {
      ...
      forecastEl.innerHTML =
        ...
        `5-session average: <strong>${forecast.ma5}</strong> · ` +
        `Trend: <strong>${slopeStr} pts/session</strong>. ` +
        `Score may reach <span class="forecast-projected-score">${forecast.projected}</span> this week. ` +
        `<span style="font-size:0.7rem;font-weight:600;color:${confColor};">Confidence: ${forecast.confidence.label}</span>`;
      forecastEl.classList.add('forecast-ready');
  ...
  if (exportCsvBtn) {
    exportCsvBtn.addEventListener('click', () => {
      ...
      sessionHistory.forEach(s => rows.push([
        s.date, s.name || 'Anonymous', s.age || '—', s.tone || '—',
        s.duration || '—', s.bpm || '—', s.face, s.breath, s.meditation,
        s.overall, s.stability !== undefined ? s.stability : '—', s.cielabUsed ? 'Yes' : 'No'
      ]));
